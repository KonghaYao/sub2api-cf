export default {
    ...
    groups: {
      ...
      groupModels: {
        ...
        empty: '该分组尚未配置模型',
        addModel: '添加模型',
        selectModel: '选择尚未关联的模型',
        add: '添加',
        addSuccess: '模型已添加到分组',
        addFailed: '添加分组模型失败',
        configuredEnabled: '已启用',
        configuredDisabled: '已停用',
        remove: '停用',
        restore: '恢复',
        removeSuccess: '模型已从分组停用',
        removeFailed: '停用分组模型失败',
        restoreSuccess: '分组模型已恢复',
        restoreFailed: '恢复分组模型失败',
        restoreGlobalDisabled: '全局模型已停用，无法恢复',
        restoreUnavailable: '模型当前不在可恢复候选中，请刷新后重试',
        catalogCompatibilityTitle: 'API 模型列表显示与排序（兼容）',
        catalogCompatibilityHint: '仅筛选和排序已可路由的模型；不会创建模型、启用账号能力或发布价格。红色项当前不会出现在 /v1/models。',
        catalogCompatibilityEnabled: '启用兼容显示过滤与排序',
        catalogInvalid: '当前不可路由',
        catalogSaved: 'API 模型列表显示设置已保存',
        catalogSaveFailed: '保存 API 模型列表显示设置失败',
        routingHint: '模型还需要可用账号能力和有效价格，才会出现在最终网关模型列表中。',
        ...
        diagnosis: { routable: '可路由', blocked: '已阻断', reasons: { group_model_missing: '未关联模型', group_disabled: '分组已停用', global_model_disabled: '全局模型已停用', group_model_disabled: '分组模型已停用', catalog_hidden: '目录中已隐藏', platform_mismatch: '分组与模型平台不匹配', active_price_missing: '缺少生效价格', no_group_account: '分组无账号', no_healthy_capable_account: '无健康且能力匹配的可调度账号' } }
      },
      sortOrder: '排序',
      columnSettings: '列设置',
