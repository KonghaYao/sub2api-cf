export default {
    ...
    groups: {
      ...
      groupModels: {
        action: '模型配置',
        title: '模型配置 · {name}',
        empty: '该分组尚未配置模型，可从当前分组账号同步',
        syncAccounts: '从分组账号同步',
        syncHint: '同步账号已确认保存的模型白名单；新增模型会进入待定价状态，不会自动猜测价格。',
        syncSuccess: '同步完成：最终同步 {synchronized} 个模型，其中 {pending_price} 个待定价',
        syncFailed: '同步分组账号模型失败',
        pendingPrice: '待定价',
        setPrice: '设置价格',
        updatePrice: '更新价格',
        pricingTitle: '模型价格（USD）',
        inputPrice: '输入价格（$/百万 Token）',
        outputPrice: '输出价格（$/百万 Token）',
        cacheReadPrice: '缓存读取价格（$/百万 Token）',
        perRequestPrice: '每次请求价格（$/次）',
        minimumReservation: '最低预扣金额（$/次）',
        priceHint: '最多支持 6 位小数。发布新价格后，旧价格会保留为历史版本。',
        publishPrice: '发布价格',
        invalidPrice: '价格必须是非负且最多 6 位小数的有效金额',
        priceSuccess: '模型价格已发布',
        priceFailed: '发布模型价格失败',
        enabled: '已启用',
        disabled: '已停用',
        disable: '停用',
        restore: '恢复',
