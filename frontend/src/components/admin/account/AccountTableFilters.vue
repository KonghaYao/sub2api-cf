<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select v-if="!cloudflareWorker" :model-value="filters.type" class="w-40" :options="tOpts" @update:model-value="updateType" @change="$emit('change')" />
    <Select :model-value="filters.status" class="w-40" :options="sOpts" @update:model-value="updateStatus" @change="$emit('change')" />
    <Select v-if="!cloudflareWorker" :model-value="filters.privacy_mode" class="w-40" :options="privacyOpts" @update:model-value="updatePrivacyMode" @change="$emit('change')" />
...
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; cloudflareWorker?: boolean }>()
...
const WORKER_PLATFORMS = new Set(['openai', 'anthropic', 'gemini', 'codex'])
const pOpts = computed(() => [
  { value: '', label: t('admin.accounts.allPlatforms') },
  ...CONCRETE_PLATFORM_OPTIONS.filter((option) => !props.cloudflareWorker || WORKER_PLATFORMS.has(String(option.value)))
])
