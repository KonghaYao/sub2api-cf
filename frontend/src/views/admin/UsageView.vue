...
import { listRequestErrors as listErrorLogs } from '@/api/admin/ops'
import type { OpsErrorLog } from '@/api/admin/ops'
import ModelDistributionChart from '@/components/charts/ModelDistributionChart.vue'; import GroupDistributionChart from '@/components/charts/GroupDistributionChart.vue'; import TokenUsageTrend from '@/components/charts/TokenUsageTrend.vue'
import EndpointDistributionChart from '@/components/charts/EndpointDistributionChart.vue'
import Icon from '@/components/icons/Icon.vue'
import type { AdminUsageLog, TrendDataPoint, ModelStat, GroupStat, EndpointStat, AdminUser } from '@/types'; import type { AdminUsageStatsResponse, AdminUsageListParams } from '@/api/admin/usage'

type AdminExplorerFilters = Omit<AdminUsageListParams, 'page' | 'page_size' | 'sort_by' | 'sort_order' | 'exact_total'> & {
...
const exportToExcel = async () => {
  ...
  try {
    ...
    while (true) {
      ...
      if (rows.length) XLSX.utils.sheet_add_aoa(worksheet, rows, { origin: -1 })
...
// Intentionally parse local dates before converting to RFC3339, matching the
// date picker's semantics elsewhere in this view.
...
const loadAdminErrors = async () => {
  ...
  try {
    const response = await listErrorLogs({
      ...
      user_id: filters.value.user_id == null ? undefined : String(filters.value.user_id),
      api_key_id: filters.value.api_key_id == null ? undefined : String(filters.value.api_key_id),
      account_id: filters.value.account_id == null ? undefined : String(filters.value.account_id),
      group_id: filters.value.group_id == null ? undefined : String(filters.value.group_id),
...
const onErrPage = (page: number) => { errPage.value = page; loadAdminErrors() }
const onErrPageSize = (pageSize: number) => { errPageSize.value = pageSize; errPage.value = 1; loadAdminErrors() }
