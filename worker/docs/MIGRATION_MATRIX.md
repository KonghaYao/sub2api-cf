...
| Usage/finance | Request ledger, aggregates, reconciliation and corrective workflows | Partial | Owner-scoped usage list/detail/stats/trend/model/API-key aggregates, subscription projections, cross-currency payment dashboard and the migration-0033 reconciliation scanner/action ledger exist with local tests. Migration 0040 adds bounded request/error metadata and protected diagnostic payload projections without replacing the immutable usage billing history. Payment anomaly acknowledge/resolve/reopen actions are guarded by RBAC, CAS and idempotency. The time-stable `snapshot-v2` aggregate test passes locally; cross-domain corrections/exports and production E2E remain. |
| Settings | Typed versioned settings, audit and KV invalidation | Partial | Core versioned read/write, normalized email-suffix policy, optimistic concurrency, idempotency, secret redaction, D1-authoritative privileged-operation step-up and KV invalidation tests pass. Migration 0039 adds typed, private admin round-tripping for email/LinuxDo/OIDC/WeChat/DingTalk/GitHub/Google signup and first-bind entitlement defaults without leaking them into public KV. The corresponding settings UI and the wider retained legacy settings surface remain. |
...
3. **Identity completion**: email verification, password reset, versioned notification-email
   ...
   Durable Object effects. Production still needs a verified Email Service sender, the defaults settings
   UI, deployed identity E2E and the optional legacy pending-account chooser.
...
5. **Operations and governance**: granular admin RBAC, a bounded immutable audit reader, payment
   ...
   exist; remaining work is broader audit sources and dashboard facts, Explorer resolve/richer timing,
...
6. **Secondary product features**: v0.19 has the model plaza, platform quotas, promotions,
   ...
   Remaining work is any retained standalone registration-time invitation reward, search extensions,
