...
| User management | List/create/detail/update/disable, password reset, persistent idempotency, metadata CAS, DO-versioned balance mutation and immutable financial history | Partial | Admin-created login-capable users, password reset with session revocation, concurrency/RPM fields and the frontend UUID adapter pass route/state/SQLite and concurrency tests. Migration 0055 projects exact balance/debt deltas for admin, redeem, affiliate, auth-source and usage-settlement mutations through the UserStateDO outbox; the `admin.users.read` route uses a stable `(occurred_at_ms,event_id)` cursor and preserves UUID event IDs. v0.34 adds an `admin.users.write` route that reads one frozen DO-ledger page per request through an environment-bound signed cursor, verifies immutable D1 rows and marks pre-0055 history complete only after full financial reconciliation. Bulk backfill orchestration is still pending. The isolated Workerd binding E2E proves password registration/login plus recovery-admin balance mutation through real D1/AuthRateLimitDO/UserStateDO/Queue bindings; production E2E and normal admin authentication remain |
...
| Usage/finance | Request ledger, user financial events, aggregates, reconciliation and corrective workflows | Partial | Owner-scoped usage list/detail/stats/trend/model/API-key aggregates, subscription projections, cross-currency payment dashboard and the migration-0033 reconciliation scanner/action ledger exist with local tests. Migration 0055 adds a Queue-fed immutable per-user balance/debt event ledger with server-derived admin actor identity and bounded tuple-cursor reads. v0.34 safely backfills pre-deployment DO history in resumable 100-row pages, including spend-debt reconstruction and exact-row conflict rejection; old ledgers cannot recover actor metadata. Migration 0040 adds bounded request/error metadata and protected diagnostic payload projections without replacing the immutable usage billing history; migration 0041 adds retained filter and aggregation dimensions. Payment anomaly and request/error resolve/reopen actions are guarded by RBAC, CAS, idempotency and immutable actor audit. The time-stable `snapshot-v2` aggregate test passes locally; bulk backfill coordination, cross-domain corrections/exports and production E2E remain. |
...
| Unified audit trail | Read-only cross-domain D1 event stream with stable cursor and allowlisted detail | Partial | Settings, RBAC, channel, auth and payment events share bounded list/detail routes behind `admin.audit.read`; channel mutations use the immutable migration-0051 audit table and preserve their real resource identity. Frontend uses cursor pagination and exposes no destructive clear action. Request/error, retention and remaining domain sources remain |
