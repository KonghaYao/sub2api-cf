...
| Model aliases/capabilities | Group-visible names, upstream override, channel mapping, endpoint and account capabilities | Partial | v0.30 resolves an active channel in the existing bounded route batch, gives exact mappings priority over suffix wildcards, expands wildcard targets, enforces `restrict_models` against mapping or channel-pricing coverage, preserves the original immutable billing snapshot and restores the public model. Migration 0052 permits mixed concrete providers only inside composite groups while retaining concrete-group integrity; account discovery, Pool membership and platform quota admission stay pinned to the deterministically resolved concrete model platform. v0.31 also resolves exact and suffix-wildcard aliases that exist only in the channel catalog, while retaining the catalog model's real provider/upstream identity and failing closed on ambiguous backing models. Repository and handler tests pass; account-level/compact-only mapping, channel-price customer billing and deployed E2E remain |
...
| Accounts/channels | Provider accounts plus normalized public channels, group links, mappings and exact pricing graphs | Partial | Existing provider CRUD, AES-GCM credentials, Pool sync and bounded health probes remain. v0.30 adds STRICT D1 channel tables and bounded admin list/create/get/update/delete with UUIDs, ETag/CAS, idempotency, audit, atomic graph replacement, catalog RBAC and a Worker frontend micros/PPM adapter. Migration 0053/v0.31 adds normalized account-stat rule graphs, scoped exact/wildcard matching, token intervals, flat request/image prices, per-account rate multipliers, immutable settlement snapshots, a bounded 15-minute account-usage rollup, RBAC-protected admin account statistics and corresponding Worker UI contracts. Unsupported cache-write/image-token rule dimensions are rejected, and `apply_pricing_to_account_stats` remains disabled until channel prices participate in customer billing. Provider quota contracts, channel-price customer billing and production E2E remain |
| Usage/finance | Request ledger, aggregates, reconciliation and corrective workflows | Partial | Owner-scoped usage list/detail/stats/trend/model/API-key aggregates, subscription projections, cross-currency payment dashboard and the migration-0033 reconciliation scanner/action ledger exist with local tests. Migration 0040 adds bounded request/error metadata and protected diagnostic payload projections without replacing the immutable usage billing history; migration 0041 adds retained filter and aggregation dimensions. Payment anomaly and request/error resolve/reopen actions are guarded by RBAC, CAS, idempotency and immutable actor audit. The time-stable `snapshot-v2` aggregate test passes locally; cross-domain corrections/exports and production E2E remain. |
...
6. **Secondary product features**: v0.19 has the model plaza, platform quotas, promotions,
   ...
   Remaining work is channel-price customer billing, any retained standalone registration-time invitation reward, search extensions,
