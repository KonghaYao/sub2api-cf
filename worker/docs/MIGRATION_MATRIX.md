...
| Available channel catalog | Authenticated, opt-in channel/group/model/price projection from normalized D1 rows | Partial | v0.30 adds a fail-closed public setting, bounded D1 batch, public-field whitelist, UUID group IDs, private/subscription/composite visibility, exact integer price conversion and Worker frontend route/table contracts. Global LiteLLM price fallback and deployed E2E remain |
...
6. **Secondary product features**: v0.19 has the model plaza, platform quotas, promotions,
   ...
   Remaining work is account-stat pricing rules, catalog-external aliases, default-price/model-sync
   helpers, any retained standalone registration-time invitation reward, search extensions,
