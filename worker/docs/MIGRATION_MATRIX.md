...
| Deployment | v0.33 replaces the fragile bulk remote apply with a fail-closed, per-file migration runner that validates continuous project/Wrangler ledgers and resumes a verified commit-before-registration interruption. v0.34 adds a verified cross-store backup bundle core and restore plan, but remote adapters are not yet implemented. Empty/checkpoint unit fixtures and isolated real Wrangler local D1 runs pass; remote staging, authenticated production smoke and rollback drills remain. | Partial |
...
| Usage and billing API | `/v1/usage` and `/v1/sub2api/billing` with immutable price version | Partial | Routes and balance/subscription billing attribution exist. Migration 0041 projects platform, request type, inbound/upstream endpoint, billing mode and native-compaction dimensions; an explicit dimensions version distinguishes rolling-deploy writes from genuine `unknown` request types while legacy rows are backfilled. Migration 0053 adds immutable standard cost, matched account-stat price, account multiplier and final account cost snapshots to every text/image settlement and Queue projection. Migration 0054 adds a bounded immutable customer-pricing snapshot without rewriting historical usage. Text reservations and settlements now freeze channel exact/longest-wildcard, interval, per-request, service-tier and timezone pricing at route time; Queue replay retains the exact decision. Account statistics use a Queue-maintained 15-minute D1 rollup plus a bounded legacy fallback, with custom scoped price over enabled channel basis over catalog standard cost. Complete legacy aggregate/filter coverage, channel image/video pricing and deployed E2E remain |
...
| User management | List/create/detail/update/disable, password reset, persistent idempotency, metadata CAS, DO-versioned balance mutation and immutable financial history | Partial | Admin-created login-capable users, password reset with session revocation, concurrency/RPM fields and the frontend UUID adapter pass route/state/SQLite and concurrency tests. Migration 0055 projects exact balance/debt deltas for admin, redeem, affiliate, auth-source and usage-settlement mutations through the UserStateDO outbox; the `admin.users.read` route uses a stable `(occurred_at_ms,event_id)` cursor and preserves UUID event IDs. v0.34 adds an `admin.users.write` route that reads one frozen DO-ledger page per request through an environment-bound signed cursor, verifies immutable D1 rows and marks pre-0055 history complete only after full financial reconciliation. New DO rows persist exact state versions and rollback tombstones; migration 0056 records immutable progress/failure/completion actor audit. Pre-v0.34 rollback gaps require manual reconciliation, and bulk orchestration is still pending. The isolated Workerd binding E2E proves password registration/login plus recovery-admin balance mutation through real D1/AuthRateLimitDO/UserStateDO/Queue bindings; production E2E and normal admin authentication remain |
...
| Accounts/channels | Provider accounts plus normalized public channels, group links, mappings and exact pricing graphs | Partial | Existing provider CRUD, AES-GCM credentials, Pool sync and bounded health probes remain. v0.30 adds STRICT D1 channel tables and bounded admin list/create/get/update/delete with UUIDs, ETag/CAS, idempotency, audit, atomic graph replacement, catalog RBAC and a Worker frontend micros/PPM adapter. Migration 0053/v0.31 adds normalized account-stat rule graphs, scoped exact/wildcard matching, token intervals, flat request/image prices, per-account rate multipliers, immutable settlement snapshots, a bounded 15-minute account-usage rollup, RBAC-protected admin account statistics and corresponding Worker UI contracts. v0.32 connects channel token/per-request prices to gateway reservation and settlement without another D1 roundtrip, enables `apply_pricing_to_account_stats`, and fails closed on ambiguous or unsupported media prices. Provider quota contracts, channel image/video prices and production E2E remain |
| Usage/finance | Request ledger, user financial events, aggregates, reconciliation and corrective workflows | Partial | Owner-scoped usage list/detail/stats/trend/model/API-key aggregates, subscription projections, cross-currency payment dashboard and the migration-0033 reconciliation scanner/action ledger exist with local tests. Migration 0055 adds a Queue-fed immutable per-user balance/debt event ledger with server-derived admin actor identity and bounded tuple-cursor reads. v0.34 safely backfills provable pre-deployment DO history in resumable 100-row pages, including spend-debt reconstruction, exact-row conflict rejection and migration-0056 actor audit; old ledgers cannot recover actor metadata, and old rollback gaps without tombstones require manual reconciliation. Migration 0040 adds bounded request/error metadata and protected diagnostic payload projections without replacing the immutable usage billing history; migration 0041 adds retained filter and aggregation dimensions. Payment anomaly and request/error resolve/reopen actions are guarded by RBAC, CAS, idempotency and immutable actor audit. The time-stable `snapshot-v2` aggregate test passes locally; bulk backfill coordination, cross-domain corrections/exports and production E2E remain. |
...
| Unified audit trail | Read-only cross-domain D1 event stream with stable cursor and allowlisted detail | Partial | Settings, RBAC, channel, auth, payment and financial-history backfill events share bounded list/detail routes behind `admin.audit.read`; channel mutations use migration 0051 and backfill attempts use migration 0056 immutable actor audit. Frontend uses cursor pagination and exposes no destructive clear action. Request/error, retention and remaining domain sources remain |
...
| Backup/restore | Export versioned D1 records and R2 manifests; verified restore workflow | Partial | v0.34 streams pre-exported D1 SQL, DO NDJSON and R2 inventory into a strict SHA-256 manifest, rejects missing/extra/tampered/unsafe entries, validates all inputs before writes and passes deterministic local byte-for-byte restore tests. Remote D1 export/import, DO restore, R2 synchronization and an empty-environment drill remain. |
