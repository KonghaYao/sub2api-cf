...
| Anthropic Messages | `/v1/messages`, token counting, Anthropic SSE/errors | Partial | Native Anthropic URL/auth/body, synchronous usage, SSE terminal/usage/cancellation, count-tokens and retry fixtures pass. v0.33 adds strict generation controls, direct-account preservation, output-effort/sampling fallback mapping and count-token stripping; signed thinking/cache-control content blocks, remaining `apicompat` variants and deployed E2E remain. |
...
| Usage/finance | Request ledger, user financial events, aggregates, reconciliation and corrective workflows | Partial | Owner-scoped usage list/detail/stats/trend/model/API-key aggregates, subscription projections, cross-currency payment dashboard and the migration-0033 reconciliation scanner/action ledger exist with local tests. Migration 0055 adds a Queue-fed immutable per-user balance/debt event ledger with server-derived admin actor identity and bounded tuple-cursor reads; it does not backfill pre-deployment DO history. Migration 0040 adds bounded request/error metadata and protected diagnostic payload projections without replacing the immutable usage billing history; migration 0041 adds retained filter and aggregation dimensions. Payment anomaly and request/error resolve/reopen actions are guarded by RBAC, CAS, idempotency and immutable actor audit. The time-stable `snapshot-v2` aggregate test passes locally; historical export/backfill, cross-domain corrections/exports and production E2E remain. |
