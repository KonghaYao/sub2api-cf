...
| Frontend compatibility | User and admin UI smoke/E2E suites pass against the Worker | Partial — v0.39 adds a passing local real-browser patrol for all 30 reachable public/user routes, subscription redemption UI and Stripe create/cancel slices, and fixes the Worker global `fetch` receiver in the Workerd outbound seam. Administrator-route patrol and deployed smoke remain |
| Deployment | v0.33 adds the fail-closed per-file D1 migration runner; v0.34 adds the verified local cross-store bundle core. v0.36 adds streaming legacy-export conversion/reconciliation, single-writer cohort plans, environment-allowlisted remote plans, empty-target proof and journaled adapter execution. v0.38 makes USER_STATE backup/restore executable; v0.39 adds the exact seven-table SUBSCRIPTION_STATE transport/adapter. POOL_STATE, AUTH_RATE_LIMIT, API_KEY_LIMIT_STATE and R2 remain contract-only. Real staging cutover, authenticated production smoke and rollback/restore drills remain. | Partial |
...
| Settings | Typed versioned settings, audit and KV invalidation | Partial | Core versioned read/write, normalized email-suffix policy, optimistic concurrency, idempotency, secret redaction, D1-authoritative privileged-operation step-up and KV invalidation tests pass. Migration 0039 adds typed, private admin round-tripping for email/LinuxDo/OIDC/WeChat/DingTalk/GitHub/Google signup and first-bind entitlement defaults without leaking them into public KV. Migration 0077 adds the original `audit_log_retention_days` field with exact GET/PATCH round-tripping, `0` permanence, range validation and the same CAS/idempotency/audit guarantees. The Worker UI now exposes these values without changing its original template or styles; the wider retained legacy settings surface remains. |
| Unified audit trail | Immutable cross-domain events plus isolated administrator request audit | Partial | Settings, RBAC, channel, account, auth, payment and financial-history events retain their category-scoped stream. Migration 0076 adds a separate paginated request log behind `admin.audit.read`, with event-time actor/auth snapshots, real method/path/route/request ID, trusted `CF-Connecting-IP`, status and latency. The original AuditLog UI and fields are restored exactly. v0.42.37 adds operations-write RBAC, user-access-only fresh TOTP, failure throttling, stable idempotency and an atomic clear transaction that preserves one truthful trace. v0.42.38 adds bounded scheduled retention of only the request-audit table, with an absolute UTC cutoff, 500-row ticks and newest-clear-trace preservation. v0.42.39 adds clone-based JSON-only request-body capture with recursive fixed-value secret redaction, conservative route denial, 256 KiB input and 16 KiB valid-JSON storage bounds, explicit placeholders and detail-only projection. Remaining domain sources remain. |
...
| Request/error explorer | D1 metadata index, R2 payload/archive, redaction and retention | Partial | Migration 0040, owner/admin route suites and gateway lifecycle hooks cover fixed tuple-seek cursors, authorization, recursive secret redaction, 96 KiB payload bounds, R2 retry, related-upstream lookup, 30-day retention, metadata repair and orphan cleanup. Admin resolve/reopen uses operations-write RBAC, CAS, idempotency, immutable server-derived actor audit and frontend conflict refresh. v0.42.40 restores the six original Ops templates/styles and the original bounded page/total, filter, search, sort, detail and related-preview contracts while retaining cursor clients; count and data share one D1 batch and unsupported contracts return typed errors. Migration 0078 captures the selected upstream endpoint, trusted Cloudflare client IP, and bounded User-Agent for new observations; deleted API-key state is derived from the retained key ID and current key relation. Pre-migration rows preserve unknown request-context values instead of fabricating them. Broader dashboard aggregates, richer timing dimensions and deployed authenticated E2E remain. |
...
| Channel monitor | Scheduled account/model tests and bounded failure actions | Partial | Generic Cron account health plus Pool removal/reactivation remains. Migration 0060/v0.37 adds manual per-model Queue inference probes, provider-native response validation, invalid-response/timeout/HTTP classification, immutable history, replay-safe firing/recovery state and bounded outbox/expired-lease recovery on real SQLite/D1. v0.38 adds the bounded Worker UI and opaque-cursor history. The old standalone `/monitor` experience is removed; silences, reports and notification delivery around the retained Worker-native probes remain. |
...
| Airwallex, standalone Alipay/WeChat Pay and EasyPay | Remove | Stripe is the sole Worker payment provider; removed routes redirect to retained outcomes |
| Legacy standalone monitor page | Remove | Bounded Worker-native account/model probes and cursorized history remain in the operations UI |
| Legacy pending-OAuth account chooser | Remove | Direct one-time OAuth callbacks and explicit authenticated account linking remain |
...
1. **Payment closure**: the Stripe-only slice now has server-priced hosted checkout, encrypted
   ...
   have per-order term accounting and exact cumulative refund compensation. v0.39 adds local
   passing browser order creation/cancellation coverage. Remaining
   work is authenticated deployed smoke plus Stripe webhook, fulfilment and refund-recovery drills.
   Airwallex, standalone Alipay/WeChat Pay and EasyPay are removed, not deferred. The order state
   machine and webhook verification are production implementations, not stubs.
...
3. **Identity completion**: email verification, password reset, versioned notification-email
   ...
   Email Service sender and deployed identity E2E. The legacy pending-account chooser is removed.
...
5. **Operations and governance**: granular admin RBAC, a bounded immutable audit reader, payment
   ...
   audit sources and dashboard facts, Explorer richer timing, administrator-route browser patrol,
   alerts and silences around the retained per-model probes, non-payment finance corrections, DLQ replay,
   cross-domain retention, the remaining DO/R2 adapters and a verified real restore drill. The legacy
   standalone monitor is removed.
