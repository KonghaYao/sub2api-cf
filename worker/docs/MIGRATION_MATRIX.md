...
| Media generation | D1 task/job metadata, replay-safe Queue workflows, R2 media/ZIP artifacts, synchronous Worker-native Images execution and exact media billing | Partial | v0.21 proves the owner-scoped batch Images flow. v0.25 covers `/v1/images/generations` and `/v1/images/edits` (including legacy aliases), bounded JSON/multipart input, direct API-key and Codex OAuth/setup-token execution, buffered output, transformed Codex SSE and byte-exact native Direct SSE. Direct streaming includes JSON/mislabeled-JSON fallback, pre-header retry, no post-commit regeneration, tolerant sidecar accounting, URL/base64 completion counting, disconnect drain, lease/monetary-hold renewal and actual-output tier charging. Unit and isolated binding suites pass. Ordinary async Images, provider-job reconciliation, video, audio/voice/live, realtime/WebSocket and deployed provider smoke remain Pending; see `LEGACY_GATEWAY_TEST_MAP.md` |
...
4. **Media and realtime**: v0.21 delivers the batch Images tracer bullet: the frontend-used
   ...
   Remaining media work is ordinary asynchronous `/images/*/async` plus
   `/images/tasks/:task_id`, provider-job result reconciliation, and all video,
