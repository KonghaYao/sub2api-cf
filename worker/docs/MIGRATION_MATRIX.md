...
| Channel monitor | Scheduled account/model tests and bounded failure actions | Partial | Generic Cron account health plus Pool removal/reactivation remains. Migration 0060/v0.37 adds manual per-model Queue inference probes, invalid-response/timeout/HTTP classification, immutable history and replay-safe firing/recovery state on real SQLite/D1. Synthetic Cron recovery, frontend controls, silences, reports and notification delivery remain. |
