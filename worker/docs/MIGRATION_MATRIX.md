...
| Accounts/channels | Provider accounts plus normalized public channels, group links, mappings and exact pricing graphs | Partial | Existing provider CRUD, AES-GCM credentials, Pool sync and bounded health probes remain. v0.30 adds STRICT D1 channel tables and bounded admin CRUD; migration 0053/v0.31 adds account-stat rules, exact/wildcard matching, token intervals, flat request/image prices, immutable settlement snapshots and rollups. v0.32 connects channel token/per-request prices to gateway settlement. v0.35 adds synchronous-image alias/wildcard tier pricing, maximum reservation, actual-output settlement and fail-closed missing/duplicate/ambiguous tiers without collapsing account costs into customer charges. Provider quota contracts, video routes/pricing and production E2E remain. |
