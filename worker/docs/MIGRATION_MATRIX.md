...
| Usage/finance | Request ledger, aggregates, reconciliation and corrective workflows | Partial | Owner-scoped usage list/detail/stats/trend/model/API-key aggregates, subscription projections, cross-currency payment dashboard and the migration-0033 reconciliation scanner/action ledger exist with local tests. Payment anomaly acknowledge/resolve/reopen actions are guarded by RBAC, CAS and idempotency. The time-stable `snapshot-v2` aggregate test passes locally; broader request/error finance views, cross-domain corrections/exports and production E2E remain. |
| Settings | Typed versioned settings, audit and KV invalidation | Partial | Core versioned read/write, normalized email-suffix policy, optimistic concurrency, idempotency, secret redaction, D1-authoritative privileged-operation step-up and KV invalidation tests pass. The wider legacy settings surface, including provider-specific signup/first-bind entitlement defaults, remains. |
...
3. **Identity completion**: email verification, password reset, versioned notification-email
   ...
   verified mailbox and password without weakening session security. Production still needs a verified
...
5. **Operations and governance**: granular admin RBAC, a bounded immutable audit reader, payment
   dashboard and payment-reconciliation actions exist; remaining work is broader audit sources and
   dashboard facts, request/error explorer, alerts and silences, per-model synthetic channel probes,
   non-payment finance corrections, DLQ replay, retention, and verified D1/R2 export-and-restore.
6. **Secondary product features**: v0.18 has the model plaza, platform quotas, promotions,
