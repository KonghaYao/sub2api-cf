...
| Same: synchronous Images, ordinary async Images and batch Images routes | Replace + Pending | v0.21 delivers Worker-native batch Images, v0.25 completes synchronous Images for direct API-key and Codex accounts, and v0.26 delivers the core ordinary async submit/poll lifecycle on D1, Queue and R2; URL rehosting and unsupported-platform status parity remain Pending |
...
and Codex synchronous execution, and v0.26 adds ordinary asynchronous Images.
...
| `service/image_storage_test.go`: offload success/failure and base64/data-URL handling | Write every successful image to R2 before completing the task, remove `b64_json` from persistent task JSON, and record a 502 failed task if offload fails; D1 must never contain image bytes or credentials | v0.26: raw request bytes, rewritten result JSON and decoded `b64_json` images live in R2; D1 keeps lifecycle/index metadata only and owner-authenticated content URLs replace base64. Data-URL and remote-URL rehosting remain Pending. |
