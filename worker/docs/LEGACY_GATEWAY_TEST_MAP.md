...
| Channel exact/wildcard mapping, restricted-model admission and public model restoration | Replace | v0.30 `test/gateway/repository.test.ts` and `test/gateway/handler.test.ts` prove exact-over-wildcard precedence, wildcard target expansion, mapping/pricing allow-list admission, inactive-channel pass-through, immutable catalog billing and one batched D1 hot-path read. v0.31 adds catalog-external exact/wildcard aliases with deterministic backing-model/provider resolution and ambiguity rejection. Account mapping, compact-only mapping and applying channel prices to customer billing remain Pending |
