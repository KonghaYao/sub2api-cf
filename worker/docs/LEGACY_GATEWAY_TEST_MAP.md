...
| Channel exact/wildcard mapping, restricted-model admission and public model restoration | Replace | v0.30 `test/gateway/repository.test.ts` and `test/gateway/handler.test.ts` prove exact-over-wildcard precedence, wildcard target expansion, mapping/pricing allow-list admission, inactive-channel pass-through and one batched D1 hot-path read. v0.31 adds catalog-external exact/wildcard aliases with deterministic backing-model/provider resolution and ambiguity rejection. v0.32 freezes exact/longest-wildcard token or per-request channel prices for reservation, settlement, Queue replay and optional account statistics while preserving the catalog `price_id`; unsupported image/video channel prices fail before billing. Account mapping, compact-only mapping, media channel pricing and deployed E2E remain Pending |
...
| `batch_image_test.go`; `batch_image_processor_test.go`: transitions, provider polling and result reconciliation | Retain `queued`, `running`, `settling`, `completed`, `failed`, `cancelled`, `output_deleted`; Queue delivery is at least once and every retained transition is CAS/idempotent. Eligible small jobs may use Gemini Batch while large jobs retain per-item execution until Files/JSONL is Worker-safe. | Replace + Pending: v0.29 covers one-create submission, safe ambiguous-create lookup, polling, exact custom-ID reconciliation, deterministic R2 materialization, terminal cancellation, lease renewal and Cron repair in `test/media/handlers.test.ts`, `test/media/gemini-batch.test.ts` and `test/media/schema.test.ts`. The rollout flag defaults off and full 200-output Files/JSONL provider jobs remain Pending. |
