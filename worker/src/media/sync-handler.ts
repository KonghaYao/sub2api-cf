...
import { normalizeNativeImageResponse, readSyncImageResponse } from './sync-provider'
 ...
 *
 * v0.22 buffers bounded JSON/multipart requests and buffered JSON responses.
 * Streaming is rejected before admission/billing and remains a later slice.
 */
export async function handleSyncImages(
  ...
  try {
    ...
    if (manifest.options.stream === true) {
      throw new GatewayError(501, 'IMAGE_STREAMING_NOT_IMPLEMENTED', 'Synchronous image streaming is not migrated yet')
    }
    ...
    if (route.candidates.some((candidate) => candidate.platform !== 'openai')) {
      throw new GatewayError(503, 'IMAGE_PROVIDER_NOT_SUPPORTED', 'No direct OpenAI Images account is configured', 'server_error')
    }
    ...
    let completed: {
      accountId: string
      normalized: ReturnType<typeof normalizeNativeImageResponse>
      publicBody: unknown
      status: number
      headers: Headers
    } | null = null
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      ...
      try {
        ...
        const upstreamBody = providerBody(
          manifest,
          route.model.upstream_name,
          context.env.SYNC_IMAGE_MODERATOR === undefined,
        )
        const plan = buildProviderRequest({
          account,
          credential,
          operation: operation === 'generations' ? 'images_generations' : 'images_edits',
          model: route.model.upstream_name,
          body: upstreamBody,
          client_headers: context.req.raw.headers,
        })
        copySafeImageHeaders(plan.headers, context.req.raw.headers)
        ...
        const normalized = normalizeNativeImageResponse(parsed)
    ...
    return new Response(JSON.stringify(completed.publicBody), {
