  ...
  try {
    ...
    const [usage, keys, recent, platforms] = await context.env.DB.batch([
      context.env.DB.prepare(`SELECT COUNT(*) total_requests, COALESCE(SUM(input_tokens),0) input_tokens, COALESCE(SUM(output_tokens),0) output_tokens,
       COALESCE(SUM(cache_read_tokens),0) cache_read_tokens, COALESCE(SUM(cache_write_tokens),0) cache_write_tokens, COALESCE(SUM(amount_micros),0) amount_micros,
       ...
       CAST(ROUND(AVG(duration_ms)) AS INTEGER) average_duration_ms FROM usage_projection WHERE user_id = ?`).bind(today,today,today,today,today,today,user.id),
    ...
    return controlSuccess({ total_api_keys: integer(k.total_api_keys), active_api_keys: integer(k.active_api_keys), total_requests:integer(u.total_requests), total_input_tokens:Math.max(0,integer(u.input_tokens)-integer(u.cache_read_tokens)-integer(u.cache_write_tokens)), total_output_tokens:integer(u.output_tokens), total_cache_creation_tokens:integer(u.cache_write_tokens), total_cache_read_tokens:integer(u.cache_read_tokens), total_tokens:totalTokens, total_cost:usd(integer(u.amount_micros)), total_actual_cost:usd(integer(u.amount_micros)), today_requests:integer(u.today_requests), today_input_tokens:Math.max(0,integer(u.today_input_tokens)-integer(u.today_cache_read_tokens)-integer(u.today_cache_write_tokens)), today_output_tokens:integer(u.today_output_tokens), today_cache_creation_tokens:integer(u.today_cache_write_tokens), today_cache_read_tokens:integer(u.today_cache_read_tokens), today_tokens:todayTokens, today_cost:usd(integer(u.today_amount_micros)), today_actual_cost:usd(integer(u.today_amount_micros)), average_duration_ms:integer(u.average_duration_ms), rpm: Math.round(integer(r.requests)/5*100)/100, tpm: Math.round(integer(r.tokens)/5*100)/100, by_platform: (platforms.results as any[]).map(platformStat) })
  } catch (error) { return controlError(asGatewayError(error)) }
}
...
function groupStatement(db: D1Database, filter: UsageFilter): D1PreparedStatement {
  return db.prepare(`SELECT u.group_id, COALESCE(g.name,'') group_name, COUNT(*) requests, COALESCE(SUM(u.input_tokens+u.output_tokens),0) tokens, COALESCE(SUM(u.amount_micros),0) amount FROM usage_projection u LEFT JOIN "groups" g ON g.id=u.group_id WHERE ${filter.where} GROUP BY u.group_id,g.name ORDER BY amount DESC LIMIT 100`).bind(...filter.values)
}
