...
export async function listUsageErrors(context: Context<Bindings>): Promise<Response> { try { await authenticateUserRequest(context.req.raw,context.env); const page=queryInteger(context.req.query('page'),'page',1,1,1_000_000), size=queryInteger(context.req.query('page_size'),'page_size',20,1,100); return controlSuccess({items:[],total:0,page,page_size:size,pages:0}) }catch(error){return controlError(asGatewayError(error))} }
export async function usageErrorDetail(context: Context<Bindings>): Promise<Response> { try { await authenticateUserRequest(context.req.raw,context.env); throw new GatewayError(404,'usage_error_not_found','Usage error details are not persisted') }catch(error){return controlError(asGatewayError(error))} }
