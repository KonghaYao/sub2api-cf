...
export async function dashboardStats(context: Context<Bindings>): Promise<Response> {
  try {
    ...
    return controlSuccess({ total_api_keys: integer(k.total_api_keys), active_api_keys: integer(k.active_api_keys), total_requests:integer(u.total_requests), total_input_tokens:Math.max(0,integer(u.input_tokens)-integer(u.cache_read_tokens)), total_output_tokens:integer(u.output_tokens), total_cache_creation_tokens:0, total_cache_read_tokens:integer(u.cache_read_tokens), total_tokens:totalTokens, total_cost:usd(integer(u.amount_micros)), total_actual_cost:usd(integer(u.amount_micros)), today_requests:integer(u.today_requests), today_input_tokens:Math.max(0,integer(u.today_input_tokens)-integer(u.today_cache_read_tokens)), today_output_tokens:integer(u.today_output_tokens), today_cache_creation_tokens:0, today_cache_read_tokens:integer(u.today_cache_read_tokens), today_tokens:todayTokens, today_cost:usd(integer(u.today_amount_micros)), today_actual_cost:usd(integer(u.today_amount_micros)), average_duration_ms:integer(u.average_duration_ms), rpm: Math.round(integer(r.requests)/5*100)/100, tpm: Math.round(integer(r.tokens)/5*100)/100, by_platform: (platforms.results as any[]).map(platformStat) })
  } catch (error) { return controlError(asGatewayError(error)) }
}
...
function stats(x:any,p?:string,endpoints:unknown[]=[]){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount_micros);return{...(p?{period:p}:{}),total_requests:integer(x.total_requests),total_input_tokens:Math.max(0,i-c),total_output_tokens:o,total_cache_tokens:c,total_cache_read_tokens:c,total_cache_creation_tokens:0,total_tokens:sum(i,o),total_cost:usd(a),total_actual_cost:usd(a),average_duration_ms:integer(x.average_duration_ms),models:{},endpoints}}
function modelStat(x:any){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount);return{model:x.model,requests:integer(x.requests),input_tokens:Math.max(0,i-c),output_tokens:o,cache_creation_tokens:0,cache_read_tokens:c,total_tokens:sum(i,o),cost:usd(a),actual_cost:usd(a)}}
...
function trendPoint(x:any,g:string,timezone:string){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount);return{date:g==='hour'?localDate(x.bucket,timezone)+' '+new Intl.DateTimeFormat('en-GB',{timeZone:timezone,hour:'2-digit',hourCycle:'h23'}).format(new Date(x.bucket))+':00':localDate(x.bucket,timezone),requests:integer(x.requests),input_tokens:Math.max(0,i-c),output_tokens:o,cache_creation_tokens:0,cache_read_tokens:c,total_tokens:sum(i,o),cost:usd(a),actual_cost:usd(a)}}
function dailyPoint(x:any,timezone:string){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount);return{date:localDate(x.bucket,timezone),requests:integer(x.requests),input_tokens:Math.max(0,i-c),output_tokens:o,cache_read_tokens:c,cache_write_tokens:0,total_tokens:sum(i,o),cost:usd(a),actual_cost:usd(a)}}
