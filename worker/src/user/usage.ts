  ...
  try {
    ...
    const [summary, endpoints] = await context.env.DB.batch([
      context.env.DB.prepare(`SELECT COUNT(*) total_requests, COALESCE(SUM(input_tokens),0) input_tokens,
        COALESCE(SUM(output_tokens),0) output_tokens, COALESCE(SUM(cache_read_tokens),0) cache_read_tokens, COALESCE(SUM(cache_write_tokens),0) cache_write_tokens,
        COALESCE(SUM(amount_micros),0) amount_micros, COALESCE(SUM(${usageBasisAmountSql()}),0) basis_amount, CAST(ROUND(AVG(duration_ms)) AS INTEGER) average_duration_ms
  ...
  try {
    ...
    const [usage, keys, recent, platforms] = await context.env.DB.batch([
      context.env.DB.prepare(`SELECT COUNT(*) total_requests, COALESCE(SUM(input_tokens),0) input_tokens, COALESCE(SUM(output_tokens),0) output_tokens,
       COALESCE(SUM(cache_read_tokens),0) cache_read_tokens, COALESCE(SUM(cache_write_tokens),0) cache_write_tokens, COALESCE(SUM(amount_micros),0) amount_micros, COALESCE(SUM(${usageBasisAmountSql()}),0) basis_amount,
       ...
       COALESCE(SUM(CASE WHEN occurred_at_ms >= ? THEN cache_write_tokens ELSE 0 END),0) today_cache_write_tokens,
       COALESCE(SUM(CASE WHEN occurred_at_ms >= ? THEN amount_micros ELSE 0 END),0) today_amount_micros,
       COALESCE(SUM(CASE WHEN occurred_at_ms >= ? THEN ${usageBasisAmountSql()} ELSE 0 END),0) today_basis_amount,
       CAST(ROUND(AVG(duration_ms)) AS INTEGER) average_duration_ms FROM usage_projection WHERE user_id = ?`).bind(today,today,today,today,today,today,today,user.id),
      ...
      context.env.DB.prepare(`SELECT COUNT(*) requests, COALESCE(SUM(input_tokens + output_tokens),0) tokens FROM usage_projection WHERE user_id = ? AND occurred_at_ms >= ?`).bind(user.id, Date.now() - 5 * 60_000),
      context.env.DB.prepare(`SELECT COALESCE(NULLIF(u.platform,''),g.platform) platform,
        ...
        COALESCE(SUM(CASE WHEN u.occurred_at_ms >= ? THEN u.input_tokens + u.output_tokens ELSE 0 END),0) today_tokens,
    ...
    const u:any=usage.results[0]??{}, k:any=keys.results[0]??{}, r:any=recent.results[0]??{}; const totalTokens=sum(u.input_tokens,u.output_tokens), todayTokens=sum(u.today_input_tokens,u.today_output_tokens)
    return controlSuccess({ total_api_keys: integer(k.total_api_keys), active_api_keys: integer(k.active_api_keys), total_requests:integer(u.total_requests), total_input_tokens:Math.max(0,integer(u.input_tokens)-integer(u.cache_read_tokens)-integer(u.cache_write_tokens)), total_output_tokens:integer(u.output_tokens), total_cache_creation_tokens:integer(u.cache_write_tokens), total_cache_read_tokens:integer(u.cache_read_tokens), total_tokens:totalTokens, total_cost:usd(integer(u.basis_amount)), total_actual_cost:usd(integer(u.amount_micros)), today_requests:integer(u.today_requests), today_input_tokens:Math.max(0,integer(u.today_input_tokens)-integer(u.today_cache_read_tokens)-integer(u.today_cache_write_tokens)), today_output_tokens:integer(u.today_output_tokens), today_cache_creation_tokens:integer(u.today_cache_write_tokens), today_cache_read_tokens:integer(u.today_cache_read_tokens), today_tokens:todayTokens, today_cost:usd(integer(u.today_basis_amount)), today_actual_cost:usd(integer(u.today_amount_micros)), average_duration_ms:integer(u.average_duration_ms), rpm: Math.round(integer(r.requests)/5*100)/100, tpm: Math.round(integer(r.tokens)/5*100)/100, by_platform: (platforms.results as any[]).map(platformStat) })
  } catch (error) { return controlError(asGatewayError(error)) }
}
  ...
  try {
    ...
    const rows = await context.env.DB.prepare(`
      SELECT ${bucket} AS bucket,
        ...
        COALESCE(SUM(amount_micros), 0) AS amount, COALESCE(SUM(${usageBasisAmountSql()}),0) basis_amount
...
const USAGE_COLUMNS=`(SELECT o.ttft_ms FROM request_observations o WHERE o.request_id=usage_projection.request_id AND o.user_id=usage_projection.user_id AND o.api_key_id IS usage_projection.api_key_id ORDER BY o.occurred_at_ms DESC,o.id DESC LIMIT 1) first_token_ms,event_id,request_id,user_id,api_key_id,account_id,COALESCE(requested_model,model) model,group_id,subscription_id,input_tokens,output_tokens,cache_read_tokens,cache_write_tokens,cache_write_5m_tokens,cache_write_1h_tokens,cache_ttl_overridden,input_amount_micros,output_amount_micros,cache_amount_micros,cache_write_amount_micros,customer_pricing_snapshot_json,base_amount_micros,amount_micros,billing_type,outcome,stream,duration_ms,occurred_at_ms,platform,request_type,inbound_endpoint,upstream_endpoint,billing_mode,native_compaction_v2,dimensions_version,image_count,image_size,image_input_size,image_output_size,image_size_source,image_size_breakdown`
...
function groupStatement(db: D1Database, filter: UsageFilter): D1PreparedStatement {
  return db.prepare(`SELECT u.group_id, COALESCE(g.name,'') group_name, COUNT(*) requests, COALESCE(SUM(u.input_tokens+u.output_tokens),0) tokens, COALESCE(SUM(u.amount_micros),0) amount, COALESCE(SUM(${usageBasisAmountSql('u.amount_micros', 'u.')}),0) basis_amount FROM usage_projection u LEFT JOIN "groups" g ON g.id=u.group_id WHERE ${filter.where} GROUP BY u.group_id,g.name ORDER BY amount DESC LIMIT 100`).bind(...filter.values)
}
...
function endpointStatement(db: D1Database, filter: UsageFilter): D1PreparedStatement {
  return db.prepare(`SELECT inbound_endpoint endpoint, COUNT(*) requests,
    ...
    COALESCE(SUM(amount_micros),0) amount, COALESCE(SUM(${usageBasisAmountSql()}),0) basis_amount FROM usage_projection
...
function usageLog(x:any){const amount=integer(x.amount_micros),componentTotal=sum(integer(x.input_amount_micros),integer(x.output_amount_micros),integer(x.cache_amount_micros),integer(x.cache_write_amount_micros),integer(x.base_amount_micros));return{id:x.event_id,user_id:x.user_id,api_key_id:x.api_key_id,account_id:x.account_id,request_id:x.request_id,model:x.model,inbound_endpoint:x.inbound_endpoint||null,group_id:x.group_id,subscription_id:x.subscription_id,input_tokens:Math.max(0,integer(x.input_tokens)-integer(x.cache_read_tokens)-integer(x.cache_write_tokens)),output_tokens:integer(x.output_tokens),cache_creation_tokens:integer(x.cache_write_tokens),cache_read_tokens:integer(x.cache_read_tokens),cache_creation_5m_tokens:integer(x.cache_write_5m_tokens),cache_creation_1h_tokens:integer(x.cache_write_1h_tokens),input_cost:usd(integer(x.input_amount_micros)),output_cost:usd(integer(x.output_amount_micros)),cache_creation_cost:usd(integer(x.cache_write_amount_micros)),cache_read_cost:usd(integer(x.cache_amount_micros)),total_cost:usd(componentTotal),actual_cost:usd(amount),rate_multiplier:componentTotal===0?1:amount/componentTotal,long_context_billing_applied:false,billing_type:x.billing_type==='subscription'?1:0,request_type:requestTypeName(x.request_type,x.stream,x.dimensions_version),stream:x.stream===1,native_compaction_v2:x.native_compaction_v2===1,duration_ms:x.duration_ms,first_token_ms:x.first_token_ms??null,image_count:integer(x.image_count),image_size:nullableText(x.image_size),image_input_size:nullableText(x.image_input_size),image_output_size:nullableText(x.image_output_size),image_size_source:nullableText(x.image_size_source),image_size_breakdown:imageBreakdown(x.image_size_breakdown),image_input_tokens:0,image_input_cost:0,image_output_tokens:0,image_output_cost:0,cache_ttl_overridden:x.cache_ttl_overridden===1,billing_mode:x.billing_mode||'token',created_at:new Date(x.occurred_at_ms).toISOString(),...usageCostDisplay(x.customer_pricing_snapshot_json)}}
...
function stats(x:any,p?:string,endpoints:unknown[]=[]){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount_micros);return{...(p?{period:p}:{}),total_requests:integer(x.total_requests),total_input_tokens:Math.max(0,i-c-integer(x.cache_write_tokens)),total_output_tokens:o,total_cache_tokens:sum(c,integer(x.cache_write_tokens)),total_cache_read_tokens:c,total_cache_creation_tokens:integer(x.cache_write_tokens),total_tokens:sum(i,o),total_cost:usd(integer(x.basis_amount??a)),total_actual_cost:usd(a),average_duration_ms:integer(x.average_duration_ms),models:{},endpoints}}
function modelStat(x:any){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount);return{model:x.model,requests:integer(x.requests),input_tokens:Math.max(0,i-c-integer(x.cache_write_tokens)),output_tokens:o,cache_creation_tokens:integer(x.cache_write_tokens),cache_read_tokens:c,total_tokens:sum(i,o),cost:usd(integer(x.basis_amount??a)),actual_cost:usd(a)}}
function groupStat(x:any){const amount=integer(x.amount);return{group_id:x.group_id,group_name:x.group_name,requests:integer(x.requests),total_tokens:integer(x.tokens),cost:usd(integer(x.basis_amount??amount)),actual_cost:usd(amount)}}
function endpointStat(x:any){const amount=integer(x.amount);return{endpoint:x.endpoint,requests:integer(x.requests),total_tokens:integer(x.tokens),cost:usd(integer(x.basis_amount??amount)),actual_cost:usd(amount)}}
...
function trendPoint(x:any,g:string,timezone:string){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount);return{date:g==='hour'?localDate(x.bucket,timezone)+' '+new Intl.DateTimeFormat('en-GB',{timeZone:timezone,hour:'2-digit',hourCycle:'h23'}).format(new Date(x.bucket))+':00':localDate(x.bucket,timezone),requests:integer(x.requests),input_tokens:Math.max(0,i-c-integer(x.cache_write_tokens)),output_tokens:o,cache_creation_tokens:integer(x.cache_write_tokens),cache_read_tokens:c,total_tokens:sum(i,o),cost:usd(integer(x.basis_amount??a)),actual_cost:usd(a)}}
function dailyPoint(x:any,timezone:string){const i=integer(x.input_tokens),o=integer(x.output_tokens),c=integer(x.cache_read_tokens),a=integer(x.amount);return{date:localDate(x.bucket,timezone),requests:integer(x.requests),input_tokens:Math.max(0,i-c-integer(x.cache_write_tokens)),output_tokens:o,cache_read_tokens:c,cache_write_tokens:integer(x.cache_write_tokens),total_tokens:sum(i,o),cost:usd(integer(x.basis_amount??a)),actual_cost:usd(a)}}
