...
type RedeemCodeStatus = 'unused' | 'processing' | 'used' | 'expired'
type RedeemCodeType = 'balance' | 'subscription'
  ...
  try {
    ...
    if (type !== undefined && type !== '') {
      if (type !== 'balance' && type !== 'subscription') {
        throw new GatewayError(400, 'unsupported_redeem_code_type', 'type must be balance or subscription')
      }
    ...
    if (status !== undefined && status !== '') {
      if (!['unused', 'processing', 'used', 'expired'].includes(status)) {
    ...
    try {
      await context.env.DB.batch([
        guardedIdempotencyInsert(
          ...
          `EXISTS (
            SELECT 1 FROM redeem_codes
             WHERE id = ? AND control_version = ? AND status IN ('unused', 'expired')
          )`,
          [id, expected],
        ),
        context.env.DB.prepare(
          `UPDATE redeem_codes
              SET control_version = CASE
                WHEN control_version = ? AND status IN ('unused', 'expired')
...
export async function batchUpdateAdminRedeemCodes(
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (
        row.status !== 'unused' ||
        (row.expires_at_ms !== null && row.expires_at_ms <= current)
      ) {
    ...
    statements.unshift(guardedIdempotencyInsert(
      ...
      batchGuardValues(rows, batch.expected, current),
    ))
    try {
      await context.env.DB.batch(statements)
    } catch (error) {
...
export async function generateAdminRedeemCodes(
  ...
  try {
    ...
    if (previous !== null) {
      ...
      return controlSuccess(replay.map((code) => ({
        ...code,
        warning: 'This idempotent operation already completed; redeem codes are not shown again.',
      })))
    }
...
function parseGenerateInput(body: Record<string, unknown>): GenerateRedeemCodesInput {
  ...
  if (type !== 'balance' && type !== 'subscription') {
    throw new GatewayError(
      ...
      'type must be balance or subscription; concurrency and invitation codes are not supported',
    )
  }
...
function parseBatchPatch(body: Record<string, unknown>): RedeemCodePatch {
  ...
  if (source.status !== undefined || source.type !== undefined || source.value !== undefined || source.value_micros !== undefined) {
    throw new GatewayError(
      400,
      'redeem_code_core_fields_immutable',
      'Redeem code status, type, and value cannot be batch updated',
    )
  }
...
function redeemCodeUpdateStatement(
  ...
  return env.DB.prepare(
    `UPDATE redeem_codes
        SET notes = ?, expires_at_ms = ?, group_id = ?,
            control_version = CASE
              WHEN control_version = ? AND status = 'unused'
...
function batchGuardExpression(rows: RedeemCodeRow[], update: boolean): string {
  const state = update
    ? `status = 'unused' AND (expires_at_ms IS NULL OR expires_at_ms > ?)`
    : `status IN ('unused', 'expired')`
...
function publicRedeemCode(row: RedeemCodeRow, current = Date.now()) {
  ...
  return {
    ...
    type: row.type,
    value: row.type === 'balance' ? row.value_micros / 1_000_000 : 0,
