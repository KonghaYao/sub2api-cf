...
export async function replaceAdminUserGroup(c: Context<{ Bindings: Env }>) {
  try {
    ...
    const group = await c.env.DB.prepare('SELECT enabled,is_exclusive,group_type FROM "groups" WHERE id=?').bind(newGroup)
    ...
    const results = await c.env.DB.batch([
      c.env.DB.prepare(`UPDATE users SET control_version=CASE WHEN control_version=? AND EXISTS
        (SELECT 1 FROM "groups" WHERE id=? AND enabled=1 AND is_exclusive=1 AND group_type='standard')
