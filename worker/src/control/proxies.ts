...
export interface ProxyConfig {
 id:number; name:string; protocol:'http'|'socks5'; host:string; port:number;
...
function publicRow(row:Row,count?:number):Record<string,unknown>{const config=JSON.parse(row.config_json) as PublicConfig;return {id:row.id,...config,expires_at:config.expires_at?new Date(config.expires_at*1000).toISOString():null,status:config.expires_at&&config.expires_at<=Date.now()/1000?'expired':config.status,control_version:row.control_version,password:null,password_configured:JSON.parse(row.config_json).password_configured===true,...(count!==undefined?{account_count:count}:{}),created_at:new Date(row.created_at_ms).toISOString(),updated_at:new Date(row.updated_at_ms).toISOString()}}
async function full(env:Env,row:Row):Promise<ProxyConfig>{const secret=await decryptCredential(row.nonce_b64,row.ciphertext_b64,env.CREDENTIALS_MASTER_KEY!,`proxy:v1:${row.creation_key}`);return {id:row.id,...JSON.parse(row.config_json),password:decodeProxyPassword(secret.api_key),control_version:row.control_version}}
...
export async function loadProxyForRequest(env:Env,id:unknown):Promise<ProxyConfig|null>{
 ...
 const chain=await env.DB.prepare(`WITH RECURSIVE chain(id,name,config_json,nonce_b64,ciphertext_b64,control_version,created_at_ms,updated_at_ms,creation_key,depth) AS (
 SELECT id,name,config_json,nonce_b64,ciphertext_b64,control_version,created_at_ms,updated_at_ms,creation_key,0 FROM proxies WHERE id=?
 UNION ALL SELECT p.id,p.name,p.config_json,p.nonce_b64,p.ciphertext_b64,p.control_version,p.created_at_ms,p.updated_at_ms,p.creation_key,c.depth+1
 FROM proxies p JOIN chain c ON p.id=json_extract(c.config_json,'$.backup_proxy_id')
 WHERE c.depth<7 AND json_extract(c.config_json,'$.fallback_mode')='proxy'
 AND (json_extract(c.config_json,'$.status')<>'active' OR (json_extract(c.config_json,'$.expires_at')>0 AND json_extract(c.config_json,'$.expires_at')<=?))
 ) SELECT * FROM chain ORDER BY depth`).bind(Number(id),Math.floor(Date.now()/1000)).all<Row>()
 const visited=new Set<number>()
 for(const row of chain.results){
  const config=JSON.parse(row.config_json) as PublicConfig
  if(visited.has(row.id))throw new GatewayError(409,'proxy_fallback_cycle','Proxy fallback configuration contains a cycle')
  visited.add(row.id)
  if(config.status==='active'&&(!config.expires_at||config.expires_at>Date.now()/1000))return full(env,row)
  if(config.fallback_mode==='direct')return null
  if(config.fallback_mode!=='proxy'||!config.backup_proxy_id)throw new GatewayError(503,'proxy_unavailable','Assigned proxy is inactive or expired','server_error')
 }
 if(chain.results.length>=8)throw new GatewayError(409,'proxy_fallback_cycle','Proxy fallback chain exceeds its limit')
 throw new GatewayError(404,'proxy_not_found','Assigned proxy or backup proxy not found')
}
...
async function normalize(env:Env,body:Record<string,unknown>,current?:ProxyConfig):Promise<{config:PublicConfig;password:string}>{
 ...
 if(candidate.protocol==='socks5h')candidate.protocol='socks5'
 if(!['http','socks5'].includes(String(candidate.protocol)))invalid('Workers currently support HTTP CONNECT and SOCKS5 proxies; HTTPS proxy TLS nesting is not supported')
...
export const listProxies=(c:C)=>reply(async()=>{
 ...
 if(c.req.query('search')){clauses.push("instr(lower(p.name||' '||json_extract(p.config_json,'$.host')||' '||COALESCE(json_extract(p.config_json,'$.username'),'')),lower(?))>0");values.push(c.req.query('search'))}
 const sortColumns:Record<string,string>={id:'p.id',name:'p.name',protocol:"json_extract(p.config_json,'$.protocol')",host:"json_extract(p.config_json,'$.host')",port:"json_extract(p.config_json,'$.port')",status:"json_extract(p.config_json,'$.status')",expires_at:"json_extract(p.config_json,'$.expires_at')",created_at:'p.created_at_ms',updated_at:'p.updated_at_ms',account_count:'account_count'}
 ...
 const items=result.results.map(row=>publicRow(row,row.account_count));if(all)return items
 return {items,total,page,page_size:size,pages:Math.ceil(total/size)}
})
export const getProxy=(c:C)=>reply(async()=>publicRow(await requireRow(c.env,c.req.param('id'))))
async function createProxyRecord(env:Env,body:Record<string,unknown>,idempotencyKey:string):Promise<Record<string,unknown>>{
 ...
 const previous=await env.DB.prepare('SELECT fingerprint,proxy_id FROM proxy_creation_requests WHERE key_hash=?').bind(key).first<{fingerprint:string;proxy_id:number}>()
 if(previous){if(previous.fingerprint!==fingerprint)throw new GatewayError(409,'idempotency_conflict','Idempotency key was used with different proxy data');return publicRow(await requireRow(env,previous.proxy_id))}
 ...
 await env.DB.batch([
  env.DB.prepare(`INSERT INTO proxies(name,config_json,nonce_b64,ciphertext_b64,creation_key,created_at_ms,updated_at_ms)
    SELECT ?,?,?,?,?,?,? WHERE NOT EXISTS(SELECT 1 FROM proxy_creation_requests WHERE key_hash=?) ON CONFLICT(creation_key) DO NOTHING`)
    .bind(normalized.config.name,JSON.stringify({...normalized.config,password_configured:normalized.password!==''}),encrypted.nonce_b64,encrypted.ciphertext_b64,key,now,now,key),
  env.DB.prepare(`INSERT INTO proxy_creation_requests(key_hash,fingerprint,proxy_id,created_at_ms)
    SELECT ?,?,id,? FROM proxies WHERE creation_key=? ON CONFLICT(key_hash) DO NOTHING`).bind(key,fingerprint,now,key),
 ])
 const recorded=await env.DB.prepare('SELECT fingerprint,proxy_id FROM proxy_creation_requests WHERE key_hash=?').bind(key).first<{fingerprint:string;proxy_id:number}>()
 ...
 return publicRow(await requireRow(env,recorded.proxy_id))
}
export const createProxy=(c:C)=>reply(async()=>createProxyRecord(c.env,await readJsonObject(c.req.raw,16384),requireIdempotencyKey(c.req.raw)),201)
export const updateProxy=(c:C)=>reply(async()=>{
 ...
 const result=await c.env.DB.prepare('UPDATE proxies SET name=?,config_json=?,nonce_b64=?,ciphertext_b64=?,control_version=control_version+1,updated_at_ms=? WHERE id=? AND control_version=?').bind(config.name,JSON.stringify({...config,password_configured:password!==''}),encrypted.nonce_b64,encrypted.ciphertext_b64,Date.now(),row.id,expected).run()
 if(!result.meta.changes)throw new GatewayError(412,'control_version_conflict','Proxy changed; reload before saving')
 return publicRow(await requireRow(c.env,row.id))
})
...
export const proxyAccounts=(c:C)=>reply(async()=>{const row=await requireRow(c.env,c.req.param('id'));const result=await c.env.DB.prepare("SELECT id,name,platform,CASE WHEN enabled=1 THEN 'active' ELSE 'inactive' END AS status FROM accounts WHERE json_extract(ui_config_json,'$.proxy_id')=? ORDER BY id").bind(row.id).all();return result.results})
...
export const batchCreateProxies=(c:C)=>reply(async()=>{
 ...
 if(!Array.isArray(body.proxies)||body.proxies.length>5)invalid('A proxy batch must contain at most five entries')
 ...
 for(const [index,value]of body.proxies.entries()){
  ...
  try{await createProxyRecord(c.env,{...proxy,name:proxy.name??`${proxy.protocol}://${proxy.host}:${proxy.port}`},`${key}:${index}`);created++}
...
export const exportProxyData=(c:C)=>reply(async()=>{
 ...
 if(ids?.length){clauses.push('id IN(SELECT value FROM json_each(?))');values.push(JSON.stringify(ids))}
  ...
  const search=c.req.query('search');if(search){clauses.push('instr(lower(name),lower(?))>0');values.push(search)}
...
export const batchDeleteProxies=(c:C)=>reply(async()=>{
 ...
 const deleted_ids:number[]=[],skipped:Array<{id:number;reason:string}>=[]
 for(const id of [...new Set(ids)]){
  try{await deleteProxyRecord(c.env,id,Number(versions[String(id)]));deleted_ids.push(id)}
  catch(error){skipped.push({id,reason:error instanceof GatewayError?error.code:error instanceof Error&&error.message.includes('proxy_in_use')?'proxy_in_use':'proxy_delete_failed'})}
 }
