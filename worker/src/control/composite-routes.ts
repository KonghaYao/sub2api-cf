...
async function requireCompositeGroup(env: Env, id: string) { const row = await env.DB.prepare(`SELECT platform FROM "groups" WHERE id=?`).bind(id).first<{platform:string}>(); if (!row) throw new GatewayError(404, 'group_not_found', 'Group was not found'); if (row.platform !== 'composite') throw new GatewayError(409, 'group_not_composite', 'Composite routes require a composite group') }
...
function parseInput(body: Record<string,unknown>): Input { const match = body.match_type === 'prefix' ? 'prefix' : body.match_type === 'exact' || body.match_type === undefined ? 'exact' : invalid('match_type'); const target = requireString(body,'target_platform',32); if (!['openai','anthropic','gemini','codex'].includes(target)) invalid('target_platform'); const publicModel = requireString(body,'public_model',256); const upstreamModel = optionalString(body,'upstream_model',256) ?? (match === 'exact' ? publicModel : ''); const endpoint = parseEndpoint(body.endpoint); const priority = optionalSafeInteger(body,'priority',1,1000000) ?? 100; const enabled = optionalBoolean(body,'enabled') ?? true; const notes = optionalString(body,'notes',1000) ?? ''; return { public_model:publicModel,match_type:match,target_platform:target,upstream_model:upstreamModel,endpoint,priority,enabled,notes } }
