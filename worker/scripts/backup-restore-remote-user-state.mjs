...
import { SUBSCRIPTION_STATE_BACKUP_V1_SCHEMA } from '../src/backup/subscription-state-backup-schema.mjs'
...
const CONTRACTS = Object.freeze({
  USER_STATE: Object.freeze({
    namespace: 'USER_STATE', artifactSchema: 'sub2api-user-state-backup',
    schema: USER_STATE_BACKUP_V1_SCHEMA, tableNames: TABLE_NAMES,
  }),
  SUBSCRIPTION_STATE: Object.freeze({
    namespace: 'SUBSCRIPTION_STATE', artifactSchema: 'sub2api-subscription-state-backup',
    schema: SUBSCRIPTION_STATE_BACKUP_V1_SCHEMA,
    tableNames: Object.freeze(SUBSCRIPTION_STATE_BACKUP_V1_SCHEMA.map(({ name }) => name)),
  }),
})

/**
 * Node-side adapter for the authenticated Durable Object Worker transport. It is
...
export function createUserStateBackupRemoteAdapter(options) {
  return createStateBackupRemoteAdapter(options, CONTRACTS.USER_STATE)
}

export function createSubscriptionStateBackupRemoteAdapter(options) {
  return createStateBackupRemoteAdapter(options, CONTRACTS.SUBSCRIPTION_STATE)
}

function createStateBackupRemoteAdapter(options, contract) {
  if (options?.environment !== 'staging' && options?.environment !== 'production') {
    throw new Error('Durable Object backup adapter environment must be staging or production')
  }
  const origin = canonicalOrigin(options.origin)
  if (origin !== ALLOWED_ORIGINS[options.environment]) {
    throw new Error('Durable Object backup adapter origin is not allow-listed for its environment')
  }
  if (typeof options.token !== 'string' || options.token.length < 32) {
    throw new Error('Durable Object backup adapter token must contain at least 32 characters')
  }
  const fetcher = options.fetcher ?? ((request) => fetch(request))
  if (typeof fetcher !== 'function') throw new Error('Durable Object backup adapter fetcher is required')
  const timeoutMs = options.timeoutMs ?? 60_000
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1_000 || timeoutMs > 300_000) {
    throw new Error('Durable Object backup adapter timeout must be between 1000 and 300000 milliseconds')
  }

  return Object.freeze({
    supports(step) {
      return isSupportedStep(step, options.environment, origin, contract)
    },
    async execute(step) {
      requireSupportedStep(step, options.environment, origin, contract)
      if (step.operation === 'export-durable-object-ndjson') {
        await exportArtifact(step, requestFor(step, origin, options.environment, options.token, timeoutMs), fetcher, contract)
      } else {
        await restoreArtifact(step, requestFor(step, origin, options.environment, options.token, timeoutMs), fetcher, contract)
      }
    ...
    async verify(step) {
      ...
      const artifact = readArtifactSummary(artifactFile.buffer, step, options.environment, contract)
      ...
      if (step.operation === 'restore-durable-object-ndjson') {
        if (integrity.bytes !== step.artifact_bytes || integrity.sha256 !== step.artifact_sha256) {
          throw new Error(`Durable Object restore artifact no longer matches its plan: ${step.id}`)
        }
      }
      ...
      if (!response.ok) throw new Error(`Durable Object remote verifier failed with HTTP ${response.status}: ${step.id}`)
      const remote = await safeJson(response, `Durable Object remote verifier returned invalid JSON: ${step.id}`)
      if (
        remote.schema !== contract.artifactSchema
        || remote.version !== 1
        || remote.environment !== options.environment
        || remote.namespace !== contract.namespace
      ...
      ) throw new Error(`Durable Object remote read-back does not match the artifact: ${step.id}`)
      return {
        status: 'verified',
        step_id: step.id,
...
async function exportArtifact(step, request, fetcher, contract) {
  const response = await fetcher(request)
  if (!response.ok || response.body === null) {
    throw new Error(`Durable Object export failed with HTTP ${response.status}: ${step.id}`)
  }
  if (!(response.headers.get('content-type') ?? '').toLowerCase().startsWith('application/x-ndjson')) {
    throw new Error(`Durable Object export returned an unsupported content type: ${step.id}`)
  }
  await writeNewFileAtomically(step.output, response.body)
  try {
    const artifactFile = await readArtifactFile(step.output)
    readArtifactSummary(artifactFile.buffer, step, step.environment, contract)
  } catch (error) {
...
async function restoreArtifact(step, request, fetcher, contract) {
  const artifactFile = await readArtifactFile(step.artifact)
  const integrity = artifactFile.integrity
  if (integrity.bytes !== step.artifact_bytes || integrity.sha256 !== step.artifact_sha256) {
    throw new Error(`Durable Object restore artifact no longer matches its plan: ${step.id}`)
  }
  const artifact = readArtifactSummary(artifactFile.buffer, step, step.environment, contract)
  ...
  if (!response.ok) throw new Error(`Durable Object restore failed with HTTP ${response.status}: ${step.id}`)
  const result = await safeJson(response, `Durable Object restore returned invalid JSON: ${step.id}`)
  if (
    result.restored !== true
    || result.inventory_digest !== artifact.inventoryDigest
    || result.state_digest !== artifact.stateDigest
  ) throw new Error(`Durable Object restore completion evidence does not match the artifact: ${step.id}`)
}
...
function isSupportedStep(step, environment, origin, contract) {
  if (
    !isRecord(step)
    ...
    || step.resource.namespace !== contract.namespace
  ...
  return step.request.path === `/internal/backup/durable-objects/${contract.namespace}/${encodeURIComponent(step.resource.objectId)}/${action}`
}

function requireSupportedStep(step, environment, origin, contract) {
  if (!isSupportedStep(step, environment, origin, contract)) {
    throw new Error(`Durable Object backup adapter does not support step: ${String(step?.id)}`)
  }
}

function canonicalOrigin(value) {
  if (typeof value !== 'string') throw new Error('Durable Object backup adapter origin is required')
  let url
  try { url = new URL(value) } catch { throw new Error('Durable Object backup adapter origin is invalid') }
  if (
    url.protocol !== 'https:' || url.username !== '' || url.password !== ''
    || url.pathname !== '/' || url.search !== '' || url.hash !== ''
  ) throw new Error('Durable Object backup adapter origin must be an HTTPS origin')
  return url.origin
}
...
async function writeNewFileAtomically(path, body) {
  ...
  try {
    await lstat(path)
    throw new Error(`Durable Object adapter output already exists: ${path}`)
  } catch (error) {
  ...
  try {
    ...
    while (true) {
      ...
      if (bytes > MAX_ARTIFACT_BYTES) throw new Error('Durable Object export exceeded the bounded artifact size')
...
function readArtifactSummary(bytes, step, environment, contract) {
  if (bytes.byteLength > MAX_ARTIFACT_BYTES) throw new Error(`Durable Object artifact is too large: ${step.id}`)
  const text = bytes.toString('utf8')
  if (!text.endsWith('\n')) throw new Error(`Durable Object artifact is truncated: ${step.id}`)
  const lines = text.slice(0, -1).split('\n')
  if (lines.length < 2 || lines.length > MAX_ROWS + 2) {
    throw new Error(`Durable Object artifact line count is invalid: ${step.id}`)
  }
  ...
  if (
    ...
    || header.schema !== contract.artifactSchema || header.version !== 1
    || header.environment !== environment || header.namespace !== contract.namespace
  ...
  ) throw new Error(`Durable Object artifact summary is invalid: ${step.id}`)

  const schemaContract = parseSchemaContract(header.schema_contract, step.id, contract)
  const tableCounts = parseTableCounts(header.tables, step.id, contract.tableNames)
  const rows = records.slice(1, -1).map((record) => parseRow(record, schemaContract, step.id))
  if (rows.length !== header.row_count) throw new Error(`Durable Object artifact row count does not match: ${step.id}`)
  validateRowOrder(rows, step.id, contract.tableNames)
  for (const table of tableCounts) {
    if (rows.filter((row) => row.table === table.name).length !== table.row_count) {
      throw new Error(`Durable Object artifact table inventory does not match: ${step.id}`)
    }
  }
  ...
  ) throw new Error(`Durable Object artifact digest does not match: ${step.id}`)
...
function parseCanonicalLine(line, stepId) {
  if (Buffer.byteLength(line, 'utf8') > MAX_LINE_BYTES) {
    throw new Error(`Durable Object artifact line exceeds the size limit: ${stepId}`)
  }
  let record
  try { record = JSON.parse(line) } catch { throw new Error(`Durable Object artifact is invalid NDJSON: ${stepId}`) }
  if (!isRecord(record) || JSON.stringify(record) !== line) {
    throw new Error(`Durable Object artifact is not canonical NDJSON: ${stepId}`)
  }
  return record
}

function parseSchemaContract(value, stepId, contract) {
  if (JSON.stringify(value) !== JSON.stringify(contract.schema)) {
    throw new Error(`Durable Object artifact schema contract is not the exact v1 schema: ${stepId}`)
  }
  if (!Array.isArray(value) || value.length !== contract.tableNames.length) {
    throw new Error(`Durable Object artifact schema contract is invalid: ${stepId}`)
  }
  return value.map((candidate, tableIndex) => {
    if (
      !isRecord(candidate)
      || !hasExactKeys(candidate, ['name', 'columns'])
      || candidate.name !== contract.tableNames[tableIndex]
...
function parseTableCounts(value, stepId, tableNames) {
  if (!Array.isArray(value) || value.length !== tableNames.length) {
    throw new Error(`Durable Object artifact table inventory is invalid: ${stepId}`)
  }
  return value.map((candidate, index) => {
    if (
      !isRecord(candidate)
      || !hasExactKeys(candidate, ['name', 'row_count'])
      || candidate.name !== tableNames[index]
...
function parseRow(value, schemaContract, stepId) {
  if (!isRecord(value) || !hasExactKeys(value, ['type', 'table', 'rowid', 'values'])) {
    throw new Error(`Durable Object artifact row is invalid: ${stepId}`)
  }
...
function validateRowOrder(rows, stepId, tableNames) {
  let tableIndex = 0
  let previousRowid = 0
  for (const row of rows) {
    const index = tableNames.indexOf(row.table)
    if (index < tableIndex) throw new Error(`Durable Object artifact rows are out of order: ${stepId}`)
    if (index > tableIndex) {
      tableIndex = index
      previousRowid = 0
    }
    if (row.rowid <= previousRowid) {
      throw new Error(`Durable Object artifact has duplicate or unordered rows: ${stepId}`)
    }
...
async function readArtifactFile(path) {
  ...
  try {
    ...
    if (!before.isFile() || before.size > BigInt(MAX_ARTIFACT_BYTES)) {
      throw new Error(`Durable Object artifact is not a bounded regular file: ${path}`)
    }
    ...
    if (
      ...
      throw new Error(`Durable Object artifact changed while it was being read: ${path}`)
    }
    return {
      buffer,
      integrity: {
