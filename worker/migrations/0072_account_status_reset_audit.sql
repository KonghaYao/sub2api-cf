...
-- audit ledgers to record administrative runtime-state resets explicitly.
...
CREATE TABLE admin_account_operation_guards (
  ...
  action TEXT NOT NULL CHECK (action IN (
    'account.enable', 'account.disable', 'account.health_probe.queue', 'account.status_reset'
  )),
...
VALUES (72, 'account_status_reset_audit', CAST(unixepoch('subsec') * 1000 AS INTEGER));
