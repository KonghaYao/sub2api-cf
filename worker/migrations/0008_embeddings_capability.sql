...
ALTER TABLE models
  ADD COLUMN embeddings INTEGER NOT NULL DEFAULT 0 CHECK (embeddings IN (0, 1));
...
CREATE INDEX idx_account_models_capability
  ON account_models(model_id, chat_completions, responses, embeddings, account_id);

DROP TRIGGER bump_gateway_revision_model_update;
CREATE TRIGGER bump_gateway_revision_model_update
AFTER UPDATE OF platform, public_name, upstream_name, endpoint, embeddings, enabled ON models
BEGIN
...
AFTER UPDATE OF account_id, model_id, chat_completions, responses, embeddings ON account_models
BEGIN
  UPDATE gateway_config_revision
     SET revision = revision + 1, updated_at_ms = NEW.updated_at_ms
   WHERE singleton = 1;
...
VALUES (8, 'embeddings_capability', CAST(unixepoch('subsec') * 1000 AS INTEGER));
