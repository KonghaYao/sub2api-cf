...
describe('upstream model sync through the Worker HTTP routes',()=>{
  ...
  it('loads test-modal models and executes the selected model as SSE instead of a health probe',async()=>{
    ...
    const created=await t.app.request('/api/v1/admin/accounts',{method:'POST',headers:{...t.headers,'idempotency-key':'model-test-account'},body:JSON.stringify({...body,name:'pucoding',max_concurrency:1,extra:{openai_responses_mode:'force_chat_completions'},credentials:{api_key:'test-upstream-secret',base_url:body.base_url,model_mapping:{'gpt-test':'gpt-test'}}})},t.env)
    expect(created.status).toBe(201);const id=(await created.json() as any).data.id
    const fetcher=vi.fn().mockResolvedValueOnce(new Response('data: {"choices":[{"delta":{"content":"OK"},"finish_reason":null}]}\n\ndata: {"choices":[{"delta":{},"finish_reason":"stop"}]}\n\ndata: [DONE]\n\n',{headers:{'content-type':'text/event-stream'}}))
    ...
    expect(fetcher.mock.calls[0][0]).toBe('https://pucoding.com/v1/chat/completions')
  ...
  it('times out while reading a stalled catalog body',async()=>{
    ...
    expect(await r.text()).toContain('upstream_model_sync_timeout')
