...
describe('proxy catalog with real SQLite', () => {
  it('preserves original empty identity fields while changing unrelated proxy settings', async () => {
    const test = fixture()
    try {
      const proxy = await test.create('preserved')
      const response = await test.request(`/proxies/${proxy.id}`, 'PUT', {
        name: '', protocol: '', host: '', port: 0, username: '', password: '', status: '', expiry_warn_days: 14,
      }, proxy.control_version)
      expect(response.status).toBe(200)
      expect(await loadProxyForRequest(test.env, proxy.id)).toMatchObject({ name: 'preserved', protocol: 'http',
        host: 'proxy.example.test', port: 8080, username: 'proxy-user', password: 'private-proxy-password',
        status: 'active', expiry_warn_days: 14, control_version: proxy.control_version + 1 })
    } finally { test.raw.close() }
  })

  it('encrypts storage and exposes credentials only through the original administrator projections', async () => {
    ...
    for (const path of ['/proxies', '/proxies/all', `/proxies/${proxy.id}`]) {
      ...
      expect(response.status).toBe(200)
      expect(await response.json()).toMatchObject({data:path===`/proxies/${proxy.id}`?{password:'private-proxy-password'}:path==='/proxies/all'?[{password:'private-proxy-password'}]:{items:[{password:'private-proxy-password'}]}})
    }
    expect(proxy).toMatchObject({ password: 'private-proxy-password', password_configured: true })
  ...
  it('serializes concurrent independent imports and keeps each replay outcome stable',async()=>{
    const test=fixture(),item={protocol:'socks5h',host:'concurrent.example.test',port:1080,username:'u',password:'secret'}
    try {
      const keys=['concurrent-import-a','concurrent-import-b']
      const outcomes=await Promise.all(keys.map(async key=>(await (await test.request('/proxies/batch','POST',{proxies:[item]},undefined,key)).json() as any).data))
      expect(outcomes.map(value=>value.created).sort()).toEqual([0,1])
      expect(outcomes.map(value=>value.skipped).sort()).toEqual([0,1])
      expect(test.raw.prepare('SELECT COUNT(*) AS n FROM proxies').get().n).toBe(1)
      for(const [index,key] of keys.entries())expect((await (await test.request('/proxies/batch','POST',{proxies:[item]},undefined,key)).json() as any).data).toEqual(outcomes[index])
      expect(JSON.stringify(test.raw.prepare('SELECT * FROM proxy_creation_requests').all())).not.toContain('secret')
    } finally {test.raw.close()}
  })

  it('finds legacy encrypted duplicates and permits different displayed copies',async()=>{
    const test=fixture()
    try {
      const original=await test.create('same display name')
      test.raw.prepare('UPDATE proxies SET identity_digest=NULL WHERE id=?').run(original.id)
      const response=await test.request('/proxies/batch','POST',{proxies:[{protocol:'socks5h',host:'proxy.example.test',port:8080,username:'proxy-user',password:'private-proxy-password'}]},undefined,'legacy-dedup')
      expect(await response.json()).toMatchObject({data:{created:0,skipped:1}})
      const copy=await test.create('same display name', {password:'different'}, 'different-copy')
      expect(copy.name).toBe(original.name);expect(copy.id).not.toBe(original.id)
      const listed=await test.request('/proxies?search=same%20display&sort_by=name')
      expect(await listed.json()).toMatchObject({data:{total:2,items:[{name:'same display name'},{name:'same display name'}]}})
    } finally {test.raw.close()}
  })
  ...
  it('requires each delete version and preserves stale or referenced proxies without pretending the batch succeeded',async()=>{
    ...
    expect(await response.json()).toMatchObject({data:{deleted_ids:[one.id],skipped:[{id:two.id,code:'control_version_conflict',reason:'Proxy changed; reload before deleting'}]}})
