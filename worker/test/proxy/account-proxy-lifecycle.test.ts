...
import * as transport from '../../src/gateway/proxy-fetch'
...
it('saved account model sync decrypts its assigned proxy, crosses CONNECT/TLS, and rejects the same binding immediately after deactivation',async()=>{
 ...
 const forbidden=await app.request('/api/v1/admin/proxies',{},env)
 expect(forbidden.status).toBe(401);expect(await forbidden.text()).not.toContain('proxy-secret')
 const inventory=await request('/proxies')
 expect(await inventory.json()).toMatchObject({data:{items:[{id,password:'proxy-secret'}]}})
 ...
 const tlsSocket:transport.ProxySocket={opened:Promise.resolve(),closed:new Promise(()=>undefined),readable:new ReadableStream({start(c){tls=c}}),writable:new WritableStream({write(bytes){tlsText+=dec.decode(bytes);const body=JSON.stringify({data:[{id:'composer-2.5',reasoning:false,input_modalities:['text'],context_window:128000}]});tls.enqueue(enc.encode(`HTTP/1.1 200 OK\r\nContent-Length: ${body.length}\r\n\r\n${body}`))}}),close:async()=>{closeCount++},startTls(){throw Error('nested TLS')}}
 const plainSocket:transport.ProxySocket={opened:Promise.resolve(),closed:new Promise(()=>undefined),readable:new ReadableStream({start(c){plain=c}}),writable:new WritableStream({write(bytes){plainText+=dec.decode(bytes);plain.enqueue(enc.encode('HTTP/1.1 200 Connection Established\r\n\r\n'))}}),close:async()=>{closeCount++},startTls({expectedServerHostname}){tlsHost=expectedServerHostname;return tlsSocket}}
 const original=transport.fetchAccountProxy
 const dial=vi.fn(async()=>plainSocket)
 const tunnel=vi.spyOn(transport,'fetchAccountProxy').mockImplementation((env,id,url,init,signal)=>original(env,id,url,init,signal,dial))
 const direct=vi.fn(()=>{throw Error('unexpected direct connection')});vi.stubGlobal('fetch',direct)
 const models=await request('/accounts/account/models/sync-upstream',{})
 ...
 const blocked=await request('/accounts/account/models/sync-upstream',{});expect(blocked.status).toBe(503);expect(tunnel).toHaveBeenCalledTimes(2);expect(dial).toHaveBeenCalledOnce();expect(direct).not.toHaveBeenCalled()
 raw.close()
})
