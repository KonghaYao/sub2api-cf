...
it.each([['anthropic','preserve'],['codex','preserve'],['anthropic','rewrite'],['anthropic','limit']] as const)('applies persisted %s provider settings (%s) through actual admission, outbound fixture and settlement',async (platform,cacheMode)=>{
 const upstreamModel='provider-forwarding-'+(platform==='codex'?'native':platform)+(cacheMode==='preserve'?'':'-'+cacheMode)
 ...
 const seeded=await request('/api/v1/admin/bootstrap',env.ADMIN_TOKEN!,{
  ...
  models:[{public_name:model+'-unused',upstream_name:upstreamModel,endpoint:'responses',input_micros_per_million:1000000,output_micros_per_million:2000000,per_request_micros:7,minimum_reservation_micros:100}],
 })
 ...
 const route=await admin('/models',{public_name:model,upstream_name:upstreamModel,platform,endpoint:'responses'})
 ...
 if(cacheMode==='rewrite')await env.DB.prepare("UPDATE system_settings SET gateway_json=json_set(gateway_json,'$.rewrite_message_cache_control',json('true')) WHERE id='global'").run()
 if(cacheMode==='limit')await env.DB.prepare("UPDATE system_settings SET gateway_json=json_set(gateway_json,'$.claude_oauth_system_prompt_blocks',?) WHERE id='global'").bind(JSON.stringify(Array.from({length:5},(_,i)=>({text:i===0?'Custom fixture expansion':'Extra fixture '+i,cache_control:true})))).run()
 try{
  const response=await request(platform==='codex'?'/v1/responses':'/v1/messages',f.api_key,platform==='codex'?{model,input:'hello',max_output_tokens:16,stream:false}:{model,system:[{type:'text',text:'Original client instructions',cache_control:{type:'ephemeral',ttl:'1h'}}],messages:cacheMode==='rewrite'?[{role:'user',content:'hello'},{role:'assistant',content:'answer'},{role:'user',content:'follow-up'}]:[{role:'user',content:'hello'}],max_tokens:16})
 ...
 }finally{await env.DB.prepare("UPDATE system_settings SET gateway_json=json_remove(gateway_json,'$.openai_codex_client_version','$.openai_codex_user_agent','$.claude_oauth_system_prompt','$.rewrite_message_cache_control','$.claude_oauth_system_prompt_blocks') WHERE id='global'").run()}
})
