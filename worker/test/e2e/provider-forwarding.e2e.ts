...
it.each(['anthropic','codex'] as const)('applies persisted %s provider settings through actual admission, outbound fixture and settlement',async platform=>{
 const model='provider-forwarding-'+platform+'-'+crypto.randomUUID()
 ...
 await admin('/accounts',{name:crypto.randomUUID(),platform,protocol:platform,auth_scheme:platform==='anthropic'?'x-api-key':'bearer',credential_kind:'oauth',image_adapter:platform==='codex'?'responses_image_tool':'direct_images',base_url:platform==='codex'?'https://upstream.e2e.invalid':'https://upstream.e2e.invalid/v1',api_key:'local-fixture-key',enabled:true,group_links:[{group_id:group.id,priority:0,weight:1}],model_capabilities:[{model_id:route.id,chat_completions:false,responses:true}]})
 await env.DB.batch([
