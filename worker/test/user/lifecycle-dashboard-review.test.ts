...
it('labels the actual local-hour bucket in the original format for a quarter-hour timezone', async () => {
  const t = await fixture()
  ...
  expect((await response.json() as any).data.trend).toMatchObject([{ date: '2026-09-04 05:00', actual_cost: 1.5 }])
})
