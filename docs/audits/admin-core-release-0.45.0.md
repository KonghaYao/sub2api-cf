...
先复现慢推理 15 秒假失败和心跳未延长空闲期限两项回归。修复后网关 56 文件/831 项通过，类型检查通过；双账号原生测试等待真实 65 秒后仍由原账号完成，无备用账号调用/故障样本，至少 3 次顺序续租，余额仅扣 27 micros 一次、预留归零。专项原生 17 项通过，最终完整原生 34 文件/98 项全部通过。部署结果待追加。
