...
新增真实 SSE 客户端解析 + D1 + 余额账本 + 管理仪表盘原生测试：输入 100，缓存 80，输出 2；客户端 cached_tokens=80，后台未缓存输入=20、总量=102，账本仅扣 88 micros 一次，预留归零。网关、用户统计、管理统计、观测等 76 文件/1001 项通过，类型检查通过。最终全量原生 33 文件/95 项通过；收尾补充 DONE 尾帧不重复发送回归，网关/用量 182 项复测通过，类型检查通过。部署结果待追加。
