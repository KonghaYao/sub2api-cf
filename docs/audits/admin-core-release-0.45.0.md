...
用户将 LLM 转发兼容性、稳定性与可靠性设为最高优先级，Prompt Audit 暂停。代理 HTTP 清理不再串行等待 socket.close 和 reader.cancel；立即发起清理并释放读取锁，避免不完成的上游关闭承诺阻塞下游取消。增加永不完成关闭/取消的故障注入测试。代理专项 21 项、真实 workerd 29 文件/76 项、类型检查通过。部署结果待追加。
